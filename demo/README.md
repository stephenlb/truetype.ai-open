...
Every answer uses one token because of how the code calls the model:

- one `forward()` pass, `logits_to_keep=MAX_NEW_TOKENS` (=1), reading `logits[:, -1, :]`
- no `generate()`, no sampling, no decode loop anywhere in `src/`
- logits gathered at the 26 A-Z token IDs, all from one token variant
...
The benchmark fails if any question exceeds the budget.
...
1. Drop health, ammo, and proximity from the state. None changes which
...
warm. Live gameplay produced the same results afterward: 5 kills / reward 160 / 2 kills
...
A batched forward still processes each prompt's full ~350-token prefix. Batching
...
evictions+20`, warm latency 7909ms, compared with 7687ms without caching, and
no error was raised.
...
cache miss already costs that. Warming moves the prefill to
...
still happened to rank correctly. Calibration was broken: every
...
(confidence 0.15) and the winner was arbitrary. Before the fix it fell on
...
- A cache miss costs one forward pass over the whole prompt, matching the uncached
  path, and keeps the prefix half of that pass's KV. An
...
Warm Doom decisions went from 357ms to 178ms.
...
immediately" as `Y` under "does the text request a refund?").
...
ammo and kill count come straight out of the engine. No screen capture or
...
python demo/doom_demo.py --watch            # open the gameplay window
```
...
OS screen grab, so it does not need macOS Screen Recording permission. The first
attempt used `chocolate-doom` and failed because it depended on screen capture.
...
### Prompt and target-selection fixes

1. Put the decisive fact on the last line of the prompt. A verbose state block
   (health, ammo, a list of visible objects) made the model answer `turn_left` on
   every tick and scored 0 kills. Moving the target bearing to the final line took a
   synthetic 5-case probe from 2/5 to 5/5.
2. Describe when each action is correct, including the
...
3. Keep the current target until another is much closer. With two monsters flanking
   the player, picking the tallest
...
The arena demo answers one question with three actions. This demo plays a whole
level, using Freedoom MAP01 by default or another IWAD/map, and puts the model in
charge of navigation and combat.
...
The engine handles deterministic work. A BFS route on the level's blocking lines
picks the waypoint, `USE` rides along with forward so doors open on contact, and
the engine selects the best loaded weapon when the current one runs dry. A short
corrective turn with the trigger held squares the crosshair before a shot.
Measured warm latency is ~175ms for combat, ~160ms when stuck, and ~130ms for
navigation across three cached prefixes.
...
turn that way." It chose a new direction every tick and shuffled at walls
...
2. Use `TURN180` with forward movement. Turning 180 and walking backward
   cancels itself out: the 180 fixes facing and then backward heads back into the
   wall. Forward walks out.
3. Give `shoot` about 20 engine tics. At 4 tics the model pressed fire and nothing
...
5. Remove the weapon line from the prompt. "The selected weapon is pistol with 30
   shots." made the model answer `shoot` on a monster 1900 units away; the same
   state without it answered `advance`. Removed.
6. Match the forward band to the state's wording. `bearing_words` says
...
7. Retry blocked forward movement before declaring the player stuck. A door needs
   about four presses. The
...
word and the 350-unit rule disagreed. Naming the band in the state as "close range,
under 350 units" or "far away, over 350 units" matched the criteria text and scored
5/5. The self-test uses the demo's own state builders, so it tests the strings sent
to the model instead of hand-copied versions.
